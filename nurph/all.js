 ...
 * Basic Usage:
 *
 * var t = jQuery.template('<div id="foo">Hello jQuery{name}, how are you jQuery{question}?  I am jQuery{me:substr(0,10)}</div>');
 *
 * jQuery(selector).append( t , {
 *     name: 'Stan',
 *     question: 'feeling',
...
(function(jQuery){
	 ...
	 */
	jQuery.template = function(html, options) {
		return new jQuery.template.instance(html, options);
	};
	 ...
	 */
	jQuery.template.instance = function(html, options) {
	 ...
	 *
	 * The default pattern looks for variables in JSP style, the form of: jQuery{variable}
	 ...
	 * jQuery.extend({ jQuery.template.re , {
	 *     myvartype: /...../g
	 * }
	 *
	 * Then when creating a template do:
	 * var t = jQuery.template("<div>...</div>", { regx: 'myvartype' });
	 */
	jQuery.template.regx = jQuery.template.instance.prototype.regx = {
	    jsp:        /\$\{([\w-]+)(?:\:([\w\.]*)(?:\((.*?)?\))?)?\}/g,
        ext:        /\{([\w-]+)(?:\:([\w\.]*)(?:\((.*?)?\))?)?\}/g,
        jtemplates: /\{\{([\w-]+)(?:\:([\w\.]*)(?:\((.*?)?\))?)?\}\}/g
	};
	 ...
	 */
	jQuery.template.regx.standard = jQuery.template.regx.jsp;
	
	/**
	 * Variable Helper Methods
	 *
	 * This is a collection of methods which can be used within the variable syntax, ie:
	 * jQuery{variable:substr(0,30)} Which would only print a substring, 30 characters in length
	 ...
	 * });
	 */
	jQuery.template.helpers = jQuery.template.instance.prototype.helpers = {
	 ...
	 */
	jQuery.extend( jQuery.template.instance.prototype, {
						...
						if (format.substr(0, 5) == "this."){
							...
							if (args) {
								...
								for(var i = 0, len = args.length; i < len; i++) {
									args[i] = args[i].replace(re, "jQuery1");
								}
	 ...
	 */
	jQuery.fn.domManip = function( args, table, reverse, callback ) {
		...
		var r = jQuery_old.domManip.apply(this, arguments);
     ...
     */
	jQuery.fn.html = function( value , o ) {
	    if (value && value.isTemplate) var value = value.apply( o );

		var r = jQuery_old.html.apply(this, [value]);
	 ...
	 */
	jQuery.fn.text = function( value , o ) {
	    if (value && value.isTemplate) var value = value.apply( o );

		var r = jQuery_old.text.apply(this, [value]);
        ...
        return {
                ...
                show: function() {
                        ...
                        element.css({
                                ...
                                top: offset.top - (listItems.length * 20), // + input.offsetHeight,
                                left: offset.left
                        }).show();
...
*/
  ...
  this.createSound = function(oOptions) {
    ...
    if (_html5OK(_tO)) {
      ...
      if(typeof _s.o._createSound == 'undefined'){
        if(typeof console != 'undefined'){console.log("No s.o._createSound available.")}
      // flash AS2
      }else if (_fV === 8) {
